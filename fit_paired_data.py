...
from data_and_encoder import read_and_embed_paired_dataset, read_precalculated_embeddings, write_dataset
from evaluate import calculate_statistics, save_statistics, plot_the_bar_graph, plot_correlation_heatmap

# GLOBALS
# set to False to overwrite existing results
...
def use_prefitted_model(transformer_path,  paired_data, embedding_diff_data):
    """
    Use pre-fitted ICA model.
    (SCA saving not implemented, as it cannot be pickled or torch.save()'d, only parameters can be saved)
    """
    report(f"Trying to read two pre-fitted models from {transformer_path}...")
    ...
    mixer1, mixer2 = read_prefitted_ICA_model_(transformer_path)
    report("Pre-fitted models downloaded. If you did not get a warning above, two models were correctly loaded.")
    # paired data
    report("Transforming a paired dataset using the pre-fitted model 1")
    X, Y = X_pair
    X_estimated = transformer.transform(X)  # not fitting, obvs
    Y_estimated = transformer.transform(Y)
    report("Paired data transformed.")
    # embedding difference
    report("Transforming a embedding difference dataset.")
    X_Y_estimated = transformer.transform(X_diff)
    report("embedding difference data transformed.")
    return (X_estimated, Y_estimated), X_Y_estimated
...
def optimize_dimension(threshold, d1, d2):
    """Optimize dimension with PCA variance explained > threshold"""
    report(f'Optimizing dimension based on PCA {np.round(threshold*100)}% variance interval...')
    d_pca = normalize(np.concatenate((d1,d2)))
    pca_ = PCA().fit(d_pca)
    explained = np.cumsum(pca_.explained_variance_ratio_)
    n_opt = np.argmax(explained > threshold) + 1
    report(f"Optimized value is {n_opt}")
    return n_opt
...
def save_fitted_models(mixer1, mixer2, options):
    report(f"Saving the two {options.method} models to {options.save_fitted}...")
    makedirs(dirname(options.save_fitted), exist_ok=True)
    base, ext = splitext(options.save_fitted)
    save_path2 = f"{base}_embedding_diff{ext}"
    try:
        with open(options.save_fitted, "wb") as f:
            pickle.dump(mixer1, f)
        with open(save_path2, "wb") as f:
            pickle.dump(mixer2, f)
    except:  # this with SCA; not possible to pickle or torch.save
        with open(options.save_fitted, "wb") as f:
            pickle.dump(mixer1.params, f)
        with open(save_path2, "wb") as f:
            pickle.dump(mixer2.params, f)
    report("Saving done.")
...
def main(options):
    ...
    # read dataset and load model OR read precalculated embeddings:
    if options.embs is not None and isfile(options.embs):
        ...
        # read dataset and encode it
        print("Reading the data, loading the model, and embedding...")
        d1, d2, ds, model = read_and_embed_paired_dataset(options)
    
    # Fit the model on embeddings (d1, d2) 
    # and another one with their differences (sentence_differences)
    sentence_differences = d1-d2
    if options.prefitted_model:  # load prefitted models, and transform the data
        print("Transforming paired embeddings and their differences with pre-calculated model.")
        (A,B), A_B = use_prefitted_model(options.prefitted_model, (d1,d2), sentence_differences)
        report(f"Resulting shape of fitted-transformed embeddings is {A.shape}")
    else:  # fit a model from scratch
        # set the dimension
        if options.dim == 0:
            report(f'Setting {options.method} dimension to {d1.shape[1]}')
            options.dim = d1.shape[1]
        elif 0 < options.dim < 1:
            options.dim = optimize_dimension(options.dim, d1,d2)
            report(f'Optimized dimension to {options.dim}')
        ...
        A_B, mixer2 = d1-d2, None #fit_model(options.method, sentence_differences, options.dim, random_init=options.random_init)

        # Save the model (or in case of SCA, just the parameters)
        if options.save_fitted:
            save_fitted_models(mixer1, mixer2, options)
    print("Calculations done. Saving results...")
...
if __name__=="__main__":
    ...
    if (isfile(options.result_path) or (exists(options.result_path) and ".hf" in options.result_path and len(listdir(options.result_path))>0)) and DO_NOT_OVERWRITE:
        ...
        exit()
    print(f"Starting paired {options.method}:", flush=True)
