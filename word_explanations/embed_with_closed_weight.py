...
def main(options):
    ...
    if not exists(options.embs):
        ...
        #df = ds.to_pandas()
        #print(df.head())
        #df['emb1'] = df.apply(lambda x: get_embedding(x[options.columns[0]], model=options.model_name))
        #df['emb2'] = df.apply(lambda x: get_embedding(x[options.columns[1]], model=options.model_name))
        #df.to_csv('output/embedded_1k_reviews.csv', index=False)
        #print(df.head())
        #ds = Dataset.from_pandas(df)
        #ds = ds.map(lambda x: {options.embedding_column_names[0] : get_embedding(x[options.columns[0]], model=options.model_name).reshape(-1)})
        #ds = ds.map(lambda x: {options.embedding_column_names[1] : get_embedding(x[options.columns[1]], model=options.model_name).reshape(-1)})
...
if __name__=="__main__":
    ...
    if (isfile(options.result_path) or (exists(options.result_path) and ".hf" in options.result_path and len(listdir(options.result_path))>0)) and DO_NOT_OVERWRITE:
        ...
        exit()
    print(f"Starting OpenAI embedding", flush=True)
